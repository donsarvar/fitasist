...
async function callKimiFallback(geminiMessages, systemPrompt) {
    ...
    for (const m of geminiMessages) {
        const textPart = m.parts?.map((p) => p.text).filter(Boolean).join("\n") || "";
        openAiMessages.push({
...
app.post(["/api/notify-admin", "/notify-admin"], async (req, res, next) => {
    try {
