...
export const MarathonCard = memo(function MarathonCard({ onOpenMarathons }: Props) {
  ...
  return (
    <button
      ...
      className="mt-4 w-full text-left rounded-[28px] p-5 bg-surface border border-border dark:border-border/10 shadow-soft active:scale-[0.98] transition-all group"
    >
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2">
            <span className="text-xl select-none leading-none">{next.flag}</span>
            <span className="px-2.5 py-0.5 rounded-full bg-brand/10 text-brand text-[9px] font-extrabold tracking-wider uppercase flex items-center gap-1">
        ...
        <div className="shrink-0 flex flex-col items-center justify-center px-3.5 py-2.5 rounded-2xl bg-brand/10 dark:bg-brand/15 border border-brand/15 min-w-[68px]">
          <span className="text-2xl font-black text-brand leading-none tracking-tight">
            {daysLeft}
          </span>
          <span className="text-[8px] font-extrabold text-brand uppercase tracking-wider mt-1 opacity-90">
            {daysUnit}
          </span>
